...
(function (global, factory) {
  ...
  function installModule (store, rootState, path, module, preserveState, preserveStateType) {
    if ( preserveStateType === void 0 ) preserveStateType = 'always';
    ...
    var moduleStateExists = moduleName && moduleName in parentState;
    if (!isRoot && (
      !preserveState ||
    ...
    if (!isRoot && preserveState && preserveStateType === 'mergeReplaceArrays' && moduleStateExists) {
      var moduleOriginalState = parentState[moduleName];

      store._withCommit(function () {
        Vue.set(parentState, moduleName, cjs(module.state, moduleOriginalState, { arrayMerge: function (target, source, options) { return source; } }));
      });
    }
    ...
    if (!isRoot && preserveState && preserveStateType === 'mergeConcatArrays' && moduleStateExists) {
      var moduleOriginalState$1 = parentState[moduleName];
      store._withCommit(function () {
        Vue.set(parentState, moduleName, cjs(module.state, moduleOriginalState$1, { arrayMerge: function (target, source, options) { return target.concat.apply(target, source); } }));
      });
    }
